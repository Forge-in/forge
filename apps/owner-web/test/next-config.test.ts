...
/**
 * Regression guard for a gap this app actually shipped with: admin sent the baseline
 * security headers and owner-web sent none, so the gym owner dashboard was framable.
 * A missing `headers()` produces no error and no visible symptom — only a test catches it.
 */
describe('owner-web next.config', () => {
