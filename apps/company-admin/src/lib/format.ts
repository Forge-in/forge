 ...
 *
 * Grouped because a console operator's actual job here is to compare a number on screen with
 * one written on a ticket or read aloud on a call, and an unbroken twelve-digit run is where
 * that comparison goes wrong. The grouping is the standard Indian 5-5 split.
 *
 * Anything that is not a +91 mobile is returned UNCHANGED rather than being coerced into the
 * pattern — a number that does not fit the shape is more useful shown raw than reformatted
 * into something that looks correct and is not.
 */
