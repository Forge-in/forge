...
describe('formatPhone', () => {
  /**
   * The console operator's real task here is comparing a number on screen against one on a
   * ticket or read aloud on a call. An unbroken twelve-digit run is where that goes wrong.
   */
  it('groups a +91 mobile into the standard 5-5 split', () => {
    expect(formatPhone('+919876543210')).toBe('+91 98765 43210');
  });

  /**
   * Anything that does not fit the shape is returned UNCHANGED. Coercing it would produce
   * something that looks correctly formatted and is not — worse than showing it raw, because
   * a wrong number that looks right is the one that gets acted on.
   */
  it.each([['9876543210'], ['+9198765432101'], ['+1 5551234567'], ['+91 98765 43210'], ['']])(
    'leaves %j untouched',
    (value) => {
