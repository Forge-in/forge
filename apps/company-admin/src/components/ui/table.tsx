...
export function Cell({
  ...
  className?: string;
  /**
   * Native tooltip. Worth having on a primitive because so many cells here truncate or
   * abbreviate: a relative "3 days ago" is what you scan, and the exact timestamp is what
   * you need the moment it matters. Putting it on the cell rather than an inner span keeps
  ...
  return (
    <div role="cell" className={cn('min-w-0', className)} title={title}>
      {children}
    </div>
  );
}
